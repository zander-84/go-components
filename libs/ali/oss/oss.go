...
func (v *Oss) NewObjectName(filename string) string {
	return v.GetPrefix() + v.conf.Helper.Unique().ID() + v.GetSuffix(filename)
}

// 获取新的完整名字
//______________________________________________________________________
func (v *Oss) NewObjectNameByUid(filename string, uid string) string {
	return v.GetPrefixById(uid) + v.conf.Helper.Unique().ID() + v.GetSuffix(filename)
}
