...
type message struct {
	queue        CQueue.Queue // 消费队列
	failQueue    CQueue.Queue // 失败队列
	surplusQ     int64        // 剩余个数
	faliSurplusQ int64        // 失败剩余个数
	cond         *sync.Cond   // 通知锁
...
func NewMessage(typ int) Message {
	this := new(message)
	this.queue = CQueue.NewQueue(typ)
	this.failQueue = CQueue.NewQueue(typ)
		...
		for this.queue.Size() > 0 {
			...
			if !messages.consumed && err != nil {
				...
				if !messages.consumed && !doSucess {
					...
					atomic.AddInt64(&this.faliSurplusQ, 1)
				}
			}
			atomic.AddInt64(&this.surplusQ, -1)
		}
